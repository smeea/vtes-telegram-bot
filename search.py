...
with (
    ...
    open("cardbase_crypt_playtest.json", "r") as crypt_playtest_file,
):
...
with (
    ...
    open("cardbase_lib_playtest.json", "r") as library_playtest_file,
):
...
def get_by_name(cardname):
    ...
    for card in crypt:
        if cardname in card["name"].lower() or cardname in letters_to_ascii(
            ...
            name = card["name"]
            cardimagename = (
                letters_to_ascii(re.sub("[\\W]", "", card["name"].lower()))
                + f"g{card['group']}"
            )
            ...
            cards.append([name, cardimagename, rulings])
    ...
    for card in library:
        if cardname in card["name"].lower() or cardname in letters_to_ascii(
            ...
            rulings = card["rulings"]
            cards.append([card["name"], cardimagename, rulings])
